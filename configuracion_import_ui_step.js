(function(){
  ...
  function panel(){
    ...
    p.innerHTML='<div id="xi_grid_head"><b>Grilla importada</b><span id="xi_grid_status"></span></div><div id="xi_grid_help">Revisá Simple / Combo una vez por SKU. Aplica tanto para Tienda Nube como para Mercado Libre.</div><div id="xi_grid_wrap"><table id="xi_grid_table"><colgroup><col style="width:16%"><col style="width:38%"><col style="width:8%"><col style="width:38%"></colgroup><thead><tr><th>SKU</th><th>Tienda Nube</th><th>Simple / Combo</th><th>Mercado Libre</th></tr></thead><tbody id="xi_grid_body"></tbody></table></div><div id="xi_grid_actions"><button class="xi_save" id="xi_grid_save">Guardar cambios</button><button class="xi_cancel" id="xi_grid_cancel">Cancelar</button></div>';
  ...
  function render(){
    ...
    for(const row of rows){
      ...
      tr.innerHTML='<td class="xi_sku">'+esc(row.sku||'SIN SKU')+'</td><td>'+cell(row.tn,'TN')+'</td><td><div class="xi_type"><label><input type="radio" name="xi_'+k+'" value="simple" '+(t==='simple'?'checked':'')+'>S</label><label><input type="radio" name="xi_'+k+'" value="bundle" '+(t==='bundle'?'checked':'')+'>C</label></div></td><td>'+cell(row.ml,'ML')+'</td>';
      tb.appendChild(tr);
    }
